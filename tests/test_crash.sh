...
# check that the provided exploit file is executable
if ! [[ -x "$2" ]]; then
    echo -e "$FAIL: $2 is not an executable (probably need to chmod 755)"
    exit 1
fi
...
if grep SIGSEGV "$STRACELOG"; then
  ...
  echo -e "$FAIL: $2 did not successfully corrupt memory"
  exit 1
fi
